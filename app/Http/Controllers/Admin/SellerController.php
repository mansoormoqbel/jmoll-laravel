...
class SellerController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            ...
            if ($users->fails()) {
                return redirect()->route('admin.seller.create')
                            ->withErrors($users)
                            ->withInput();
            }
            ...
            return redirect()->route('admin.seller.index')->with(['error' => $th]);
        }
    ...
    public function update(Request $request)
    {
        ...
        if ($users->fails()) {
            return redirect()->route('admin.seller.edit',$request->id)
                        ->withErrors($users)
                        ->withInput();
        }
        ...
        return redirect()->route('admin.seller.index')->with('success', 'User updated successfully.');
        ...
        try {
            ...
            if (!$user){
               /*  return ()->json(['status'=>0,'error'=>'User not find']); */
                return redirect()->route('admin.seller.index')->with(['error' => 'User not find']);
            }
           ...
           //$user -> update(['status' =>$status ]);
            ...
            return redirect()->route('admin.seller.index')->with(['success' => ' User Has Been Change Successfully!']);

        } catch (\Exception $ex) {
           /*  return response()->json(['status'=>0,'error'=>'Something went wrong, please try again later']); */
            
            return redirect()->route('admin.seller.index')->with(['error' => 'Something went wrong, please try again later']);
        }
    }
    /**
    ...
    public function destroy($id)
    {
        ...
        try {
            ...
            return redirect()->route('admin.seller.index')->with('error', 'User has been delete successfully!');
